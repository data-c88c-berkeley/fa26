...
/* Loader for published source-file viewer pages (see viewer.py).
 *
 * Concatenated after highlight.js core (+ the scheme grammar) into the
 * code-view.js served beside each project's index.html. The raw source file
 * beside the viewer page is the only copy of the code: the page fetches it
 * and highlights it here, so the text students read is byte-identical to
 * the file they download.
 */

/* highlight.js ships no Gleam grammar; this small one covers the course's
 * Gleam subset (keywords, strings, numbers, types, // comments). */
hljs.registerLanguage("gleam", function (hljs) {
  return {
    ...
    contains: [
      ...
      { className: "number", begin: "\\b[0-9][0-9_]*(\\.[0-9]+)?\\b" },
...
function sourceView(file) {
  var code = document.getElementById("source-view-code");
  fetch(file)
    ...
    .catch(function (error) {
      code.textContent =
        ...
        "Use the raw link above to view the file directly.";
    });
}
